...
![image-20210523164814254](Sping笔记.assets/image-20210523164814254.png)
...
![image-20210523164840285](Sping笔记.assets/image-20210523164840285.png)
...
![image-20210523164850808](Sping笔记.assets/image-20210523164850808.png)
...
![image-20210523164907392](Sping笔记.assets/image-20210523164907392.png)
...
![image-20210523164923593](Sping笔记.assets/image-20210523164923593.png)
...
![image-20210523164937241](Sping笔记.assets/image-20210523164937241.png)
...
![image-20210523164952978](Sping笔记.assets/image-20210523164952978.png)

使用注解需要导入context约束，增加注解的支持!

![image-20210523165001763](Sping笔记.assets/image-20210523165001763.png)
...
2. 属性如何注入
   ...
   ```JAVA
   @Component
   //等价于  <bean id="user" class="com.hello.pojo.User"/>
   public class User {
...
4. 自动装配
   ...
   @Component：自动装配通过名字、类型   组件，放在类上说明这个类被spring管理了，就是bean
   ```
...
![image-20210523165018477](Sping笔记.assets/image-20210523165018477.png)
...
![image-20210523165027787](Sping笔记.assets/image-20210523165027787.png)
...
<aop:config>
    <!--  切入点 pointcut    表达式 expression（要执行的位置 ）   -->
    <aop:pointcut id="pointcut" expression="execution(* com.Le.service.userServiceImpl.*(..))"/>
...
```xml
<bean id="diy" class="com.Le.diy.DiyPointCut"/>
<aop:config>
    <!--   自定义切面   ref：要引用的位置  -->
    <aop:aspect ref="diy">
        <!--    切入点        -->
        <aop:pointcut id="point" expression="execution(* com.Le.service.userServiceImpl.*(..))"/>
        <!--      通知      -->
...
```xml
 <!-- 开启注解支持   -->
    ...
    <bean id="annotation" class="com.Le.diy.annotation"/>
```
...
public class annotation {
    @Before("execution(* com.Le.service.userServiceImpl.*(..))")
    public void before(){
        System.out.println("方法执行前");
    }

    @After("execution(* com.Le.service.userServiceImpl.*(..))")
    public void after(){
    ...
    @Around("execution(* com.Le.service.userServiceImpl.*(..))")
...
![image-20210523165038902](Sping笔记.assets/image-20210523165038902.png)
...
5. 将自己写的实现类，注入到spring中
   ...
   ```xml
   <bean id="userMapper" class="com.Le.mapper.userMapperImpl">
       <property name="sqlSessionTemplate" ref="SqlSession"/><!--引用sqlsessiopn -->
   </bean>
   ```
